...
function RegisterScreen({ onDone, onLogin, T }) {
  ...
  const sendOtp = async () => {
    setErr("");
    if (!form.email.trim() || !form.email.includes("@")) { setErr("Enter a valid email first."); return; }
    if (!form.name.trim()) { setErr("Enter your name first."); return; }
    setOtpLoad(true);
    const { ok, data } = await apiFetch("/api/auth/send-otp", {
      method:"POST", body:JSON.stringify({ email:form.email.trim(), name:form.name.trim() })
    });
    setOtpLoad(false);
    if (!ok) { setErr(data.error || "Could not send OTP."); return; }
    setOtpSent(true);
    setNote(`OTP sent to ${form.email.trim()}. Check inbox and spam.`);
  };

  // ── Verify OTP ──
  const verifyOtp = async () => {
    setErr("");
    if (otp.length !== 4) { setErr("Please enter the 4-digit OTP."); return; }
    setOtpLoad(true);
    const { ok, data } = await apiFetch("/api/auth/verify-otp", {
      method:"POST", body:JSON.stringify({ email:form.email.trim(), otp })
    });
    setOtpLoad(false);
    if (!ok) { setErr(data.error || "Invalid OTP."); return; }
    setOtpOk(true); setNote("✓ Email verified successfully!");
  };
    ...
    <div style={{ minHeight:"100vh", display:"flex", flexDirection:"column", position:"relative", zIndex:1 }}>
      ...
      <div style={{ flex:1, padding:"0 20px 40px", maxWidth:440, margin:"0 auto", width:"100%" }}>
        ...
        <div className="glass" style={{ padding:22, marginBottom:16 }}>
          ...
          <div style={{ marginBottom:6 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <input className="input-field" type="email" placeholder="your@email.com"
                style={{ flex:1 }} value={form.email}
                onChange={e => f("email", e.target.value)} disabled={otpOk} />
              {!otpOk && (
                <button onClick={sendOtp} disabled={otpLoad || !form.email.trim() || !form.name.trim()}
                  style={{ flexShrink:0, padding:"0 14px", height:48, borderRadius:10,
                    background:"var(--gold)", border:"none",
                    color: T.dark ? "#09090f" : "#fff",
                    fontSize:12, fontWeight:700, cursor:"pointer",
                    opacity:(otpLoad || !form.email.trim()) ? 0.5 : 1 }}>
                  {otpLoad ? "…" : otpSent ? "Resend" : "Get OTP"}
                </button>
              )}
              {otpOk && (
                <div style={{ display:"flex", alignItems:"center", paddingRight:6, color:"#50b868", fontSize:22 }}>✓</div>
              )}
            </div>
          </div>
          ...
          {/* OTP box */}
          {otpSent && !otpOk && (
            <div style={{ marginBottom:14, padding:"14px", borderRadius:10,
              background: T.dark ? "rgba(201,169,110,0.06)" : "rgba(160,100,0,0.05)",
              border:`1px solid ${T.dark ? "rgba(201,169,110,0.20)" : "rgba(160,100,0,0.14)"}` }}>
              <label className="input-label">4-digit OTP from your email</label>
              <div style={{ display:"flex", gap:8 }}>
                <input className="input-field"
                  style={{ flex:1, letterSpacing:"0.35em", fontSize:22, textAlign:"center" }}
                  placeholder="· · · ·" maxLength={4} inputMode="numeric"
                  value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g,"").slice(0,4))} />
                <button onClick={verifyOtp} disabled={otpLoad || otp.length !== 4}
                  style={{ flexShrink:0, padding:"0 16px", borderRadius:10,
                    background:"var(--gold)", border:"none",
                    color: T.dark ? "#09090f" : "#fff",
                    fontSize:13, fontWeight:700, cursor:"pointer",
                    opacity:(otpLoad || otp.length !== 4) ? 0.5 : 1 }}>
                  {otpLoad ? "…" : "Verify"}
                </button>
              </div>
              <div style={{ fontSize:11, color:"var(--muted)", marginTop:8 }}>Check your inbox and spam folder. OTP expires in 10 minutes.</div>
            </div>
          )}
        ...
        <button className="btn-gold" onClick={submit}
          disabled={!canSubmit || (otpSent && !otpOk)}
          style={{ opacity:(!canSubmit || (otpSent && !otpOk)) ? 0.45 : 1, marginBottom:12 }}>
          {otpSent && !otpOk ? "⚠ Verify your email first" : "✦ Generate My Birth Chart"}
        </button>
        {!otpSent && canSubmit && (
          <p style={{ textAlign:"center", fontSize:12, color:"var(--muted)", lineHeight:1.5 }}>
            Click "Get OTP" to verify your email, then submit.
          </p>
        )}
