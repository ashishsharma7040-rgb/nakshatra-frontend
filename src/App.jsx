...
// Each theme: bg gradient + all UI colors guaranteed to be visible
const THEMES = {
  dawn: {
    // 5–8am: soft pink sunrise
    bg:          'linear-gradient(160deg,#fce4ec 0%,#f8bbd0 40%,#e8a8c8 70%,#d090b8 100%)',
    midnight:    '#fce4ec', deep:'#f8d0e0', card:'rgba(255,245,250,0.94)',
    border:      'rgba(160,80,120,0.2)',
    text:        '#280a1e', muted:'rgba(40,10,30,0.5)',
    gold:        '#9a5020', gold2:'#c07038', rose:'#b03060', sky:'#404898',
    glow:        'rgba(154,80,32,0.12)',
    // Nav — always dark enough to see icons
    navBg:       'rgba(40,10,30,0.92)',  navBorder:'rgba(200,130,160,0.3)',
    navText:     '#f8d0e0',              navActive:'#f0a060',
    // Header
    headerBg:    'rgba(40,10,30,0.88)',  headerText:'#f8d0e0',
    // Chat bubbles
    aiBg:        'rgba(255,245,250,0.96)', aiText:'#280a1e',
    userBg:      'rgba(40,10,30,0.12)',   userText:'#280a1e',
    // Input
    inputBg:     'rgba(255,245,250,0.9)', inputText:'#280a1e', inputPlaceholder:'rgba(40,10,30,0.4)',
    // Buttons
    btnBg:       'linear-gradient(135deg,#9a5020,#c07038)', btnText:'#fff',
    // Logout / utility
    utilBg:      'rgba(40,10,30,0.15)', utilText:'#280a1e', utilBorder:'rgba(40,10,30,0.2)',
    label:       '🌸 Dawn',
  },
  morning: {
    // 8–5pm: warm cream sunshine
    bg:          'linear-gradient(160deg,#fdf6ec 0%,#f5e8d0 40%,#ece0c4 70%,#e4d4b4 100%)',
    midnight:    '#fdf6ec', deep:'#f5e8d8', card:'rgba(255,252,244,0.95)',
    border:      'rgba(140,100,50,0.18)',
    text:        '#241404', muted:'rgba(36,20,4,0.45)',
    gold:        '#8a5818', gold2:'#b07830', rose:'#904050', sky:'#304880',
    glow:        'rgba(138,88,24,0.1)',
    navBg:       'rgba(30,18,4,0.90)',  navBorder:'rgba(180,130,60,0.3)',
    navText:     '#f5e8d0',             navActive:'#f0b050',
    headerBg:    'rgba(30,18,4,0.86)',  headerText:'#f5e8d0',
    aiBg:        'rgba(255,252,244,0.97)', aiText:'#241404',
    userBg:      'rgba(36,20,4,0.09)',    userText:'#241404',
    inputBg:     'rgba(255,252,244,0.93)', inputText:'#241404', inputPlaceholder:'rgba(36,20,4,0.38)',
    btnBg:       'linear-gradient(135deg,#8a5818,#b07830)', btnText:'#fff',
    utilBg:      'rgba(36,20,4,0.1)',   utilText:'#241404',  utilBorder:'rgba(36,20,4,0.18)',
    label:       '☀️ Morning',
  },
  evening: {
    // 5–8pm: deep amber sunset
    ...
    border:      'rgba(220,140,50,0.22)',
    text:        '#f5daa8', muted:'rgba(245,218,168,0.48)',
    gold:        '#f0a020', gold2:'#f5c050', rose:'#e04860', sky:'#6070b0',
    glow:        'rgba(240,160,32,0.18)',
    navBg:       'rgba(14,5,0,0.96)',   navBorder:'rgba(240,160,50,0.3)',
    navText:     '#f5daa8',             navActive:'#f5c050',
    headerBg:    'rgba(14,5,0,0.94)',   headerText:'#f5daa8',
    aiBg:        'rgba(36,16,0,0.94)',  aiText:'#f5daa8',
    userBg:      'rgba(245,218,168,0.09)', userText:'#f5daa8',
    inputBg:     'rgba(28,12,0,0.88)',  inputText:'#f5daa8', inputPlaceholder:'rgba(245,218,168,0.35)',
    btnBg:       'linear-gradient(135deg,#c05020,#f0a020)', btnText:'#0e0500',
    utilBg:      'rgba(245,218,168,0.08)', utilText:'#f5daa8', utilBorder:'rgba(240,160,50,0.25)',
    label:       '🌅 Evening',
  },
  night: {
    // 8pm–5am: deep cosmic night
    bg:          'linear-gradient(160deg,#09090f 0%,#0f0f1e 50%,#13131f 100%)',
    midnight:    '#09090f', deep:'#0f0f1e', card:'#16162a',
    border:      'rgba(255,255,255,0.07)',
    text:        '#e8e4da', muted:'rgba(232,228,218,0.45)',
    gold:        '#c9a96e', gold2:'#e8cc9a', rose:'#c97b9a', sky:'#7ba8c9',
    glow:        'rgba(201,169,110,0.18)',
    navBg:       'rgba(9,9,15,0.97)',   navBorder:'rgba(255,255,255,0.08)',
    navText:     '#e8e4da',             navActive:'#c9a96e',
    headerBg:    'rgba(9,9,15,0.92)',   headerText:'#e8e4da',
    aiBg:        'rgba(22,22,42,0.96)', aiText:'#e8e4da',
    userBg:      'rgba(255,255,255,0.06)', userText:'#e8e4da',
    inputBg:     'rgba(22,22,42,0.85)', inputText:'#e8e4da', inputPlaceholder:'rgba(232,228,218,0.32)',
    btnBg:       'linear-gradient(135deg,#c9a96e,#e8cc9a)', btnText:'#09090f',
    utilBg:      'rgba(255,255,255,0.05)', utilText:'rgba(232,228,218,0.6)', utilBorder:'rgba(255,255,255,0.08)',
    label:       '🌙 Night',
  },
};

// Global theme accessor — used by components that can't receive props
let _theme = 'night';
const getT = () => THEMES[_theme] || THEMES.night;

// ── Google Fonts + Dynamic Styles ─────────────────────────────────────────────
const GlobalStyles = ({ theme = 'night' }) => {
  ...
  return <style>{`
    ...
    *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
    :root{
      --midnight:${t.midnight}; --deep:${t.deep}; --void:${t.midnight};
      --card:${t.card}; --border:${t.border}; --gold:${t.gold};
      --gold2:${t.gold2}; --rose:${t.rose}; --sky:${t.sky};
      --glow:${t.glow}; --text:${t.text}; --muted:${t.muted}; --radius:16px;
    }
    body{background:${t.midnight};color:${t.text};font-family:'Jost',sans-serif;}
    ...
    .stars-bg{position:fixed;inset:0;pointer-events:none;z-index:0;overflow:hidden;background:${t.bg};}
    .star{position:absolute;border-radius:50%;background:rgba(255,255,255,0.9);}
    .dot{width:6px;height:6px;border-radius:50%;background:${t.gold};display:inline-block;animation:dotPulse 1.4s ease-in-out infinite;}
    .dot:nth-child(2){animation-delay:0.2s;}.dot:nth-child(3){animation-delay:0.4s;}
    .scroll-area{overflow-y:auto;scrollbar-width:thin;scrollbar-color:${t.gold} ${t.deep};}
    .divider{height:1px;background:linear-gradient(90deg,transparent,${t.gold},transparent);opacity:0.35;margin:20px 0;}
    .input-wrap{position:relative;margin-bottom:16px;}
    .input-label{display:block;font-size:11px;letter-spacing:0.12em;text-transform:uppercase;color:${t.gold};margin-bottom:8px;font-family:'Jost',sans-serif;font-weight:500;}
    .glass-input{width:100%;padding:14px 16px;background:${t.inputBg};border:1px solid ${t.border};border-radius:var(--radius);color:${t.inputText};font-size:15px;font-family:'Jost',sans-serif;outline:none;transition:border-color 0.2s;}
    .glass-input::placeholder{color:${t.inputPlaceholder};}
    .glass-input:focus{border-color:${t.gold};}
    input[type=date],input[type=time]{color-scheme:${theme === 'morning' || theme === 'dawn' ? 'light' : 'dark'};}
    .card{background:${t.card};border:1px solid ${t.border};border-radius:var(--radius);backdrop-filter:blur(12px);}
    .btn-gold{width:100%;padding:16px;border:none;border-radius:var(--radius);background:${t.btnBg};color:${t.btnText};font-size:13px;font-family:'Cinzel Decorative',serif;letter-spacing:0.1em;cursor:pointer;transition:opacity 0.2s,transform 0.15s;font-weight:400;}
    .btn-gold:hover:not(:disabled){opacity:0.88;transform:translateY(-1px);}
    .btn-gold:disabled{opacity:0.4;cursor:not-allowed;}
    .bubble-ai{background:${t.aiBg};color:${t.aiText};border-radius:4px 18px 18px 18px;padding:14px 18px;font-family:'Cormorant Garamond',serif;font-size:17px;line-height:1.7;max-width:85%;box-shadow:0 2px 16px rgba(0,0,0,0.12);animation:messageIn 0.4s ease forwards;}
    .bubble-user{background:${t.userBg};color:${t.userText};border:1px solid ${t.border};border-radius:18px 4px 18px 18px;padding:12px 18px;font-size:15px;max-width:80%;align-self:flex-end;animation:messageIn 0.4s ease forwards;}
    /* NAV — always clearly visible regardless of theme */
    ...
    .plan-card{border:1px solid ${t.border};border-radius:var(--radius);padding:20px;cursor:pointer;transition:all 0.25s;position:relative;overflow:hidden;background:${t.card};}
    .plan-card:hover{border-color:${t.gold};transform:translateY(-2px);}
    .plan-card.selected{border-color:${t.gold};background:rgba(201,169,110,0.07);}
    .plan-badge{position:absolute;top:12px;right:12px;background:${t.gold};color:${t.btnText};font-size:9px;font-weight:700;letter-spacing:0.1em;text-transform:uppercase;padding:3px 8px;border-radius:100px;}
    .credit-badge{display:inline-flex;align-items:center;gap:6px;background:rgba(201,169,110,0.12);border:1px solid rgba(201,169,110,0.25);border-radius:100px;padding:6px 14px;color:${t.gold2};font-size:12px;font-family:'Jost';}
  `}</style>;
};
...
const ChartScreen = ({ onAskAI }) => {
  ...
  return (
    <div style={{ position: 'relative', zIndex: 1, paddingBottom: 90 }}>
      ...
      <div style={{ padding: '52px 24px 20px', background: 'linear-gradient(180deg, var(--card) 0%, transparent 100%)' }}>
...
const ChatScreen = ({ credits, setCredits, onBuyCredits }) => {
  ...
  const send = async (text) => {
    ...
    try {
      ...
      // Safe JSON parse — Render sometimes returns HTML error page when sleeping
      const raw = await res.text();
      let data = {};
      try { data = JSON.parse(raw); }
      catch {
        // Server returned HTML (sleeping/crashed) — give friendly message
        setMessages(prev => [...prev, { role: 'ai', text: '🌙 The server is waking up from sleep. Please wait 30 seconds and try again — this only happens once after long inactivity.' }]);
        setLoading(false);
        return;
      }
      ...
      if (!res.ok) {
        if (res.status === 402) {
          setMessages(prev => [...prev, { role: 'ai', text: '✦ You have used all your reading credits. Please purchase more to continue your cosmic journey.' }]);
          setLocalCredits(0);
          LS.set('nakshatra_credits', '0');
        } else if (data.error === 'NO_CHART') {
          setMessages(prev => [...prev, { role: 'ai', text: '🔮 Please generate your birth chart first — go to the Chart tab to enter your birth details.' }]);
        } else {
      ...
      setMessages(prev => [...prev, { role: 'ai', text: `⚠️ ${err.message}. Please try again in a moment.` }]);
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', position: 'relative', zIndex: 1 }}>
      {/* Header */}
      <div style={{ padding: '52px 20px 16px', borderBottom: '1px solid var(--border)', background: 'var(--card)', backdropFilter: 'blur(12px)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          <div className="cinzel" style={{ fontSize: 13, color: 'var(--gold2)' }}>✦ Celestial Guide</div>
      ...
      {/* Input */}
      {localCredits !== 0 && (
        <div style={{ padding: '12px 16px 16px', borderTop: '1px solid var(--border)', background: 'var(--card)', backdropFilter: 'blur(12px)' }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            ...
            <button onClick={() => send(input)} disabled={loading || !input.trim()}
              style={{ width: 44, height: 44, borderRadius: '50%', background: 'var(--gold)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, color: 'var(--midnight)', opacity: loading || !input.trim() ? 0.5 : 1 }}>
              {Icon.send}
            </button>
          </div>
        </div>
